...
def preprocess(root_path, video_path):
    # split video to frames and mkdir
    lr_video_path = video_path
    sr_video_path = osp.join(root_path, "sr_video")

    # video = osp.join(root_path, video_path)
    ...
    video_name = osp.basename(lr_video_path).split(".")[0]  # get video name
    ...
    if not osp.exists(tempAudioFileName):
        os.system('ffmpeg -y -i "{}" -c:a copy -vn {}'.format(lr_video_path, tempAudioFileName))
